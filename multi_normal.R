...
load(file = "Desktop/Mar2018/bead_data_stained.rda")

cell_code = cell_data %>% select(`140Ce`, `142_FlagTag`, `151_CD123`, `153_CD44`, `165_KLF1`, `175_CD71`) %>% 
  head(5000) %>% as.matrix()
dim(cell_code)

cell_code1 = rnorm(n=5000, mean = 0, sd =1)
cell_code2 = rnorm(n=5000, mean = 1, sd =0.5)

cell_code = data.frame(V1 = cell_code1, V2=cell_code2) %>% as.matrix()

cmp_model = stan_model(file = "Research/RBC/Data Analysis/Mix_Gaussian_March2018/multi_normal.stan")

fit_Yule_func = function(xx, cmp_model){
  
  num_chains = 4

  data_for_stan = list(N = nrow(xx), n_prot = ncol(xx), y = xx) 
  
  run_fit = sampling(cmp_model, iter = 500, warmup = 250, data = data_for_stan, chains = num_chains, thin = 1, cores = 14)
  
  run_fit
}

run_fit = fit_Yule_func(cell_code, cmp_model)

cell_code %>% ggplot(aes(x=V1, y =V2)) + geom_point()
cell_code %>% as.data.frame() %>% ggplot(aes(x=V1)) + geom_histogram()
