...
data_t1 = state_data %>% select(-state) %>% as.matrix() %>% t() #-`2`, `4`, `6`, `8`, `10`
data_t2 = state_data %>% select(`10`, `12`, `14`) %>% as.matrix() %>% t() 
data_t3 = state_data %>% select(`14`, `16`, `18`, `20`) %>% as.matrix() %>% t() 

n_state = ncol(data_t1)
n_time = c(nrow(data_t1), nrow(data_t2), nrow(data_t3))

y = ham_dist %>% as.matrix() #%>% select(-state) 

fit_Yule_func = function(cmp_model, data_t, n_state, n_time, y){

  num_chains = 4
  
  data_for_stan = list(n_state = n_state, n_time = n_time, x_t_in = data_t, y = y) 
  
  run_fit = sampling(cmp_model, iter = 2000, warmup = 1000, data = data_for_stan, chains = num_chains, thin = 1, cores = 14)
...
run_fit1 = fit_Yule_func(cmp_model, data_t1, n_state, n_time[1], y)
run_fit2 = fit_Yule_func(cmp_model, data_t2, n_state, n_time[2], y)
run_fit3 = fit_Yule_func(cmp_model, data_t3, n_state, n_time[3], y)
...
x_t_1 = rstan::summary(run_fit1, par = "x_t_1") 
x_t_2 = rstan::summary(run_fit2, par = "x_t_1") 
x_t_3 = rstan::summary(run_fit3, par = "x_t_1") 

x_t_1_new = x_t_1$summary
x_t_2_new = x_t_2$summary
x_t_3_new = x_t_3$summary

predict1 = x_t_1_new %>% as.tibble(rownames = "prob") %>%  select(prob, `2.5%`, `50%`, `97.5%`)
predict2 = x_t_2_new %>% as.tibble(rownames = "prob") %>%  select(prob, `2.5%`, `50%`, `97.5%`)
predict3 = x_t_3_new %>% as.tibble(rownames = "prob") %>%  select(prob, `2.5%`, `50%`, `97.5%`)

predict1 = predict1 %>% mutate(prob = str_replace_all(prob, "x_t_1\\[|\\]", "")) %>% 
  separate(prob, c("tp", "state"), ",") %>% mutate(tp = as.numeric(tp))
predict2 = predict2 %>% mutate(prob = str_replace_all(prob, "x_t_1\\[|\\]", "")) %>% 
  separate(prob, c("tp", "state"), ",") %>% mutate(tp = as.numeric(tp)+n_time[1]-1)
predict3 = predict3 %>% mutate(prob = str_replace_all(prob, "x_t_1\\[|\\]", "")) %>% 
  separate(prob, c("tp", "state"), ",") %>% mutate(tp = as.numeric(tp)+n_time[1]+n_time[2]-2)

predict_data = rbind(predict1) #, predict2, predict3)

predict_data %>% ggplot(aes(x = 2*tp, y = `50%`)) +
  geom_line() + 
  geom_ribbon(aes(ymin= `2.5%`, ymax =`97.5%`), alpha = 0.2, fill ="blue") +
  geom_point(data = state_data %>% gather(tp, prob_val, -state), aes(x = as.numeric(tp), y = prob_val), colour = "red") + 
